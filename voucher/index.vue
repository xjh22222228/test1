<template>
  <div class="mk-order-vrList" >
    ...
    <div class="bottom" v-if="effectiveList.length">
      <div class="voucher-confirm-bar">
        <div class="count">{{ memberOrderVoucherCount ? $t('COUPON.select_count_voucher', { count: memberOrderVoucherCount }) : $t('COUPON.please_select') }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      // 暂存选中的代金券，用于未确认返回时恢复数据。
      tempMemberOrderVoucher: {}
    };
  },
  computed: {
    ...
    // 选中的代金券数量
    memberOrderVoucherCount() {
      if (this.memberOrderVoucher?.id) {
        return 1
      } else {
        return 0
      }
  ...
  methods: {
    ...
    onSelected(data) {
      if (this.isChecked(data)) {
        return this.$store.commit("memberOrderVoucher", {});
      }
      const { common, delivery } = this.memberOrderRedpack
      const condition1 = common?.id && common.useType === 0
      const condition2 = delivery?.id && !delivery.useWithVoucherRedpack
      // 紅包互斥
      if (condition1 || condition2) {
        ...
        let content = ''
        if (condition1 && condition2) {
          content = this.$t('userVoucher.voucher_cannot_be_used_with_coupon_all', { deliverRpAmount: delivery?.amount, amount: common?.amount })
        } else if (condition1) {
          content = this.$t('userVoucher.voucher_cannot_be_used_with_coupon', { amount: common?.amount })
        } else if (condition2) {
          content = this.$t('userVoucher.voucher_cannot_be_used_with_delivery_coupon', { deliverRpAmount: delivery?.amount })
        }
        this.configData = {
          ...this.configData,
          content
        }
        this.tempVoucher = data;
        return;
      }
      this.$store.commit("memberOrderVoucher", data);
    },
    isChecked(data) {
      return this.memberOrderVoucher?.id === data.id
    },
    beforeClose({ action, done }) {
      if (action === "confirm") {
        this.$store.commit("memberOrderVoucher", this.tempVoucher);
      }
      this.tempVoucher = {};
      done();
    },
    onBack() {
      this.$store.commit('memberOrderVoucher', this.tempMemberOrderVoucher)
      this.$store.dispatch('goBack')
    },
    onConfirm() {
      this.tempMemberOrderVoucher = {
        ...this.memberOrderVoucher
      }
      const { common, delivery } = this.memberOrderRedpack
      if (this.memberOrderVoucher?.id) {
        // 已选代金券與紅包互斥
        ...
        if (!delivery?.useWithVoucherRedpack) {
          // 清除不可叠加的免配红包
          this.$store.commit('memberOrderRedpack', {
            ...this.memberOrderRedpack,
            delivery: {}
  ...
  mounted() {
    this.$toast.clear()
    // 暂存
    this.tempMemberOrderVoucher = cloneDeep(this.memberOrderVoucher)
  }
};
</script>
